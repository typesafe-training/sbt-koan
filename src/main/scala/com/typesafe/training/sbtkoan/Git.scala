...
class Git(repository: Repository) {
  ...
  def fetch(remote: String, ref: String) = {
    val refSpec = new RefSpec(s"+refs/heads/$ref:refs/remotes/origin/$ref")
    val command =
      jgit.fetch setRemote remote setRefSpecs refSpec setCredentialsProvider credentialsProvider setTagOpt TagOpt.NO_TAGS
    command.call()
  }
  ...
  def resetHard(ref: String): Unit = {
    val command = jgit.reset setRef ref setMode ResetType.HARD
    command.call()
  }
}
